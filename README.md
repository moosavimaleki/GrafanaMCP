...
- خروجی TOON فشرده و ستونی برای کاهش مصرف توکن agent

تمام ابزارها فقط‌خواندنی هستند؛ این پروژه داشبورد، alert، datasource یا
دادهٔ مانیتورینگ را تغییر نمی‌دهد.

پاسخ ابزارها فقط یک نسخهٔ متنی برمی‌گردانند و payload را در
`structuredContent` تکرار نمی‌کنند. آرایه‌های هم‌شکل در TOON نام ستون‌ها را
یک‌بار می‌نویسند؛ برای مثال `series[2]{name,value}:` دو سری با دو ستون است.
Query آنی به‌طور پیش‌فرض فقط نام، labelها، مقدار و زمان را برمی‌گرداند. برای
آمار کامل بازه از `detail=stats` استفاده کنید و با `max_series` اندازهٔ پاسخ را
محدود نگه دارید.
...
بررسی کنید. برای لاگ‌ها از LogSQL استفاده کنید. ابزار `grafana_help` این
راهنماها، workflow پیشنهادی و توضیح قالب خروجی را فقط هنگام نیاز برمی‌گرداند؛
پاسخ عادی query دیگر ورودی درخواست یا لینک مستندات را تکرار نمی‌کند.
